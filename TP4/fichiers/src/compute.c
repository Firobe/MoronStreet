...
int monter_max (int i_d, int j_d, int i_f, int j_f)
{
  ...
  #pragma omp parallel for schedule(dynamic,42)
  for (int i = i_d; i >= i_f; i--)
    for (int j = j_d; j >= j_f; j--)
      if (img (i,j)) {
	Uint32 m = MAX(img (i+1,j), img (i,j+1));
...
{    
  ...
  for (int i=0; i < GRAIN; i++)
    for (int j=0; j < GRAIN; j++)
      #pragma omp task depend(out: cellulem[GRAIN-i-1][GRAIN-j-1]) depend(in: cellulem[GRAIN-i][GRAIN-j-1], cellulem[GRAIN-i-1][GRAIN-j])
      lancer_monte (GRAIN-i-1, GRAIN-j-1);

  #pragma omp single
  for (int i=0; i < GRAIN; i++)
    for (int j=0; j < GRAIN; j++)
      #pragma omp task depend(out: celluled[i][j]) depend(in: celluled[i-1][j], celluled[i][j-1])
      lancer_descente (i, j);
  }
