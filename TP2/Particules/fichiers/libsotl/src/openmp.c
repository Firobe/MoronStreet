...
static void omp_bounce (sotl_device_t *dev)
{
  ...
  int c;
  int i;
  for(i=0;i<set->natoms;++i){
    for(c=0;c<3;c++){
      if( (set->pos.x[i+c*set->offset] <= domain->min_ext[c]) ||  (set->pos.x[i+c*set->offset] >= domain->max_ext[c]) )
	set->speed.dx[i+c*set->offset] *= -1;
    }
  }
}
