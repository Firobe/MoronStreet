...
cl_mem tex_buffer, cur_buffer, next_buffer, cur_stagnate, next_stagnate;
...
void ocl_init (void)
{
	...
	//Stagnating buffers
	cur_stagnate = clCreateBuffer (context, CL_MEM_READ_WRITE, sizeof(unsigned) * (DIM / TILEX) * (DIM / TILEY),
			NULL, NULL);
	if (!cur_stagnate)
		exit_with_error ("Failed to allocate input stagnating buffer");

	next_stagnate = clCreateBuffer (context, CL_MEM_READ_WRITE, sizeof(unsigned) * (DIM / TILEX) * (DIM / TILEY),
			NULL, NULL);
	if (!next_stagnate)
		exit_with_error ("Failed to allocate output stagnating buffer");
...
void ocl_send_image (unsigned *image)
{
	...
	//Initialize stagnation buffers to 0
	unsigned i = 0;
	err = clEnqueueFillBuffer (queue, cur_stagnate, &i, sizeof(unsigned),
			0, sizeof(unsigned) * (DIM / TILEX) * (DIM / TILEY),
			0, NULL, NULL);
	check (err, "Failed to write to cur_stagnate");

	err = clEnqueueFillBuffer (queue, next_stagnate, &i, sizeof(unsigned),
			0, sizeof(unsigned) * (DIM / TILEX) * (DIM / TILEY),
			0, NULL, NULL);
	check (err, "Failed to write to next_stagnate");
